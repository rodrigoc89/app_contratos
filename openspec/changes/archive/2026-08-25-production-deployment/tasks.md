...
> ~~8.2~~ **Relocated to Phase 9** (immediately before 9.3) — it shares a single GREEN (9.4) with 9.3, both targeting the not-yet-created `verificarRestauracion.integration.spec.ts`. Left as a pointer, not a checkbox: the task itself is tracked and ticked at its Phase 9 position, and a checkbox here would either count as outstanding work forever or claim work was done at a position where none was.
