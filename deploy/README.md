...
- [x] A deliberately broken `/etc/contratos/api.env` (missing `DATABASE_URL`) is refused before the service stops, and the previous version is still serving traffic afterward — drilled on the live host: `deploy.sh` printed `missing required configuration variable 'DATABASE_URL' … aborting before touching the running service`, and `contratos-api` stayed `active` with its uptime counter unbroken (never stopped), `/salud` answering throughout. `api.env` restored immediately afterwards.
...
- [x] `certbot renew --force-renewal --cert-name <real hostname>` followed by `openssl s_client -connect <real hostname>:443 -servername <real hostname> </dev/null 2>/dev/null | openssl x509 -noout -dates` shows a **new** `notBefore`, proving `renewal-hook-nginx.sh` actually reloaded nginx and not just that certbot wrote a new file to disk — drilled on the live host: `notBefore` moved from `Aug 24 19:33:11` to `Aug 25 00:38:37` and the serial changed, and the **served** certificate over the wire is the new one, which is what proves `renewal-hook-nginx.sh` actually reloaded nginx rather than certbot merely writing a file. nginx's master PID is unchanged, as a reload should leave it. Note: certbot reports `Hook 'deploy-hook' ran with error output:` followed by `nginx: … test is successful` — that is `nginx -t` writing its success message to stderr, which certbot labels as error output. Alarming, harmless.
...
- [ ] After 31+ real daily runs, exactly 30 remote copies remain and the 31st-oldest is gone — six remote artifacts as of 2026-08-28, counted by `backup.sh --prune-only`'s own retention line; this one is a matter of time.
- [x] `deploy/backup.sh --prune-only` run by hand against the real remote behaves identically to its mocked-`rclone` test — same head/tail split, no accidental deletion of a retained copy — **observed 2026-08-28**. Both retention checks reported the under-limit branch and deleted nothing: `[skip] remote retention: 6 artifact(s) at or under the 30-copy limit` and `[skip] local retention: 2 copy/copies at or under the 2-copy limit`, with `/var/backups/contratos-offsite` holding 2 archives before and 2 after. Note the shape of this evidence: an under-limit run exercises the head/tail split's *guard*, not its deletion arm — the accidental-deletion half stays unproven against the real remote until there are more than 30 remote copies, which is the retention item above.
- [ ] `sudo stat -c '%U:%G %a %n' /etc/contratos /etc/contratos/gnupg /var/backups/contratos-offsite` prints `root:root 700` for all three, and `systemctl list-timers contratos-backup.timer` shows the timer `provision.sh` enabled with a next firing — **partially observed**: `/etc/contratos/gnupg` and `/var/backups/contratos-offsite` are `root:root 700` (the latter was found `755`, created by `backup.sh`'s own `mkdir -p` before `provision_backup_units` existed, and was tightened by hand), and the timer lists a next firing. `/etc/contratos` itself is `root:contratos 750` on this host because it was created by hand before provision made it; a fresh host gets `root:root 700`. Re-checked 2026-08-28: unchanged, and the timer still lists a next firing. Tightening it by hand was deliberately **not** done — unlike `/var/backups/contratos-offsite`, this directory holds the environment files the units read, so a wrong guess here costs a service that will not start on the next boot, and the only gain is cosmetic alignment with a baseline a fresh host produces on its own. It belongs in a maintenance window with a service restart to confirm, not in a drive-by.
- [x] Before `/etc/contratos/backup.env` exists, `sudo systemctl start contratos-backup.service` fails and `journalctl -u contratos-backup.service` shows `backup.sh: configuration file '/etc/contratos/backup.env' does not exist` — a loud refusal, never `226/NAMESPACE` and never silence — **drilled 2026-08-28**. `backup.env` was moved aside on the live host, the unit started by hand, and the journal read back:

  ```
  backup.sh: configuration file '/etc/contratos/backup.env' does not exist
  contratos-backup.service: Main process exited, code=exited, status=1/FAILURE
  ```

  `systemctl start` exited 1 and `is-failed` printed `failed` — the loud refusal, not `226/NAMESPACE` and not silence. The file was restored by an `EXIT` trap and verified by checksum (`sha256` identical before and after, permissions still `root:root 600`), so the drill could not leave the host without its backup configuration even if the run had died midway. `systemctl reset-failed` was run afterwards: a unit left in `failed` would mask the next real failure, which is the same defect this checklist item exists to prevent.
